...
Object.keys(blockTypes).forEach(function(type) {
  ...
  hexo.extend.tag.register(type, function(args, content) {
    ...
    var html = content
      .replace(/^### (.+)$/gm, function(_, t) { return '<h4>' + escHTML(t) + '</h4>'; })
      .replace(/^## (.+)$/gm, function(_, t) { return '<h3>' + escHTML(t) + '</h3>'; })
      .replace(/^# (.+)$/gm, function(_, t) { return '<h2>' + escHTML(t) + '</h2>'; })
      .replace(/\*\*(.+?)\*\*/g, function(_, t) { return '<strong>' + escHTML(t) + '</strong>'; })
      .replace(/\*(.+?)\*/g, function(_, t) { return '<em>' + escHTML(t) + '</em>'; })
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, function(_, t, u) { return '<a href="' + escHTML(u) + '" target="_blank" rel="noopener noreferrer">' + escHTML(t) + '</a>'; })
      .replace(/^- (.+)$/gm, function(_, t) { return '<li>' + escHTML(t) + '</li>'; })
      .replace(/\n\n/g, '</p><p>')
      .replace(/\n/g, '<br>');
    html = '<p>' + html + '</p>';
